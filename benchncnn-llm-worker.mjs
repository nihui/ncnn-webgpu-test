...
async function createRuntime(options = {}) {
    const factory = (await import(assetUrl(`./assets/benchncnn-llm-${backend}.mjs`))).default;
    return factory({
        locateFile(path) {
...
function parseResult(line) {
    const match = line.match(/^\s*(\S+)\s+([0-9.]+)\s+([0-9.]+)\s*$/);
    if (!match) return null;

    return {
        decodeTps: Number.parseFloat(match[3]),
        name: match[1],
        prefillTps: Number.parseFloat(match[2]),
    };
}
...
async function runBenchmark(message) {
    ...
    const handleLine = (text, isError) => {
        ...
        if (isError) console.warn(`[benchncnn-llm:${backend}] ${line}`);
        else console.log(`[benchncnn-llm:${backend}] ${line}`);
    ...
    await createRuntime({
        ...
        onAbort(reason) {
            rejectExit(new Error(`benchncnn_llm aborted: ${reason}`));
        },
        onExit(status) {
            if (status === 0) resolveExit();
            else rejectExit(new Error(`benchncnn_llm exited with status ${status}`));
        },
        print(text) {
    ...
    if (results.length !== 1) throw new Error(`benchncnn_llm produced ${results.length} of 1 benchmark results`);
    return results;
}
